...
bool compileShader(std::string shader, int type, int & handle)
{
	...
	if(shaderID == 0)
	{
		printf("Could not not create shader of type %s\\n", type);
		return false;
	}
	...
	if(status == 0)
	{
		...
		printf("Error compiling shader of type %d: %s\n", type, logString);
		return false;
	}
...
bool loadTexture(char * fileName, int & handle)
{
	...
	if(!success)
	{
		printf("FAILURE TO LOAD FILE THROUGH SDL2 %s\n", fileName); 
		return false;
	}
	...
	if(err != GL_NO_ERROR)
	{
		printf("FAILURE TO INITIALIZE IMAGE IN OPENGL. Returned: %d\n", err);
		return false;
	}
	free(rgbaRef);
		...
		vector<vertexData> & vertexBuffer, bool & hasUV, bool & hasNormals)
{
	...
	if(fp == NULL)
	{
		printf("OBJ: %s could not be opened\n",  fileName);
		return false;
	}
	...
	for(int i = 0; i < mtlFiles.size(); i++)
	{
		...
		if(fp == NULL)
		{
			printf("MTL FILE : %s could not be opened\\n", mtlFiles[i].c_str());
			return false;
		}
